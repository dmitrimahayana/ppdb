<?php
class Seleksi1smk_model extends CI_Model{
    ...
    function get_namax($table, $id_diterima){
        $sql = "SELECT max(NILAI_AKHIR) NA_MAX FROM `$table` where diterima =? and diterima <> -2";
    ...
    {
        $sql = "SELECT min(NILAI_AKHIR) NA_MIN FROM `$table` where diterima=? and diterima <> -2";
		$query = $this->db->query($sql,array($id_diterima));
